...
class LLMProcessor:
    """
    Simplified LLM Processor with focused prompts for better JSON extraction
    """
    ...
    def process_texts(self, extracted_texts: List[str], template_id: int) -> Dict[str, Any]:
        """
        Process extracted texts with specified template - ULTRA SIMPLIFIED VERSION
        """
        try:
            logger.info(f"🧠 PROCESSING WITH TEMPLATE ID: {template_id}")
            ...
            prompt = self._create_simple_prompt(combined_text, template_id)
            
            # Execute extraction
            structured_data = self._execute_simple_extraction(prompt, template_id)
            ...
            logger.info(f"✅ Successfully extracted data for template {template_id}")
            
            return structured_data
            
        except Exception as e:
            logger.error(f"❌ LLM processing failed for template {template_id}: {str(e)}")
            # Return a fallback structure instead of raising exception
            return self._get_fallback_structure(template_id, str(e))
    ...
    def _combine_texts(self, extracted_texts: List[str]) -> str:
        ...
        # Aggressive truncation for reliability
        if len(combined_text) > 8000:
            logger.warning(f"⚠️ Text too long ({len(combined_text)} chars), truncating to 8000")
            combined_text = combined_text[:8000] + "... [text truncated]"
    ...
    def _create_simple_prompt(self, text: str, template_id: int) -> str:
        """Create ultra-simple prompt"""
        ...
        if template_id == 1:
            ...
            focus = "private equity fund"
        else:
            schema = {
                "portfolio_summary": {
                    "assets_under_management": "number or null",
                    "number_of_companies": "number or null"
                },
                "investments": [
                    {
                        "company_name": "string",
            ...
            focus = "portfolio summary"
        
        schema_str = json.dumps(schema, indent=2)
        
        prompt = f"""
        Extract financial data for {focus} from the document below.
        ...
        - Use null for missing data
        - For numbers: extract only explicit numbers like $100M, 50%, etc.
        - For dates: format as YYYY-MM-DD if found
        - Keep it simple - only extract what you clearly see
        
        JSON:
        """
        
        logger.info(f"📝 Simple prompt length: {len(prompt)} characters")
        return prompt

    def _execute_simple_extraction(self, prompt: str, template_id: int) -> Dict[str, Any]:
        """Execute extraction with ultra-simple approach"""
        for model_config in self.available_models:
            model_name = model_config["name"]
            ...
            try:
                ...
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system", 
                            "content": "You are a data extraction assistant. Extract data from financial documents and return ONLY valid JSON. No explanations, no additional text."
                        },
                        {
                            "role": "user",
                            "content": prompt
                ...
                result_text = response.choices[0].message.content
                logger.info(f"📄 Raw response length: {len(result_text)}")
                
                # Try to parse the response
                structured_data = self._parse_json_response(result_text)
                
                if structured_data and self._is_valid_structure(structured_data):
                    logger.info(f"✅ Model {model_name} produced valid data")
                    return structured_data
                else:
                    logger.warning(f"⚠️ Model {model_name} produced invalid JSON")
                    # Try one more time with even simpler approach
                    return self._extract_minimal_data(prompt, model_name)
                    
            except Exception as e:
                logger.warning(f"❌ Model {model_name} failed: {e}")
                continue
        
        # If all models fail, return minimal structure
        logger.error("❌ All models failed, returning fallback")
        return self._get_fallback_structure(template_id, "All models failed")
    ...
    def _parse_json_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            brace_match = re.search(r'\{[^{}]*\}', cleaned_text)
            if brace_match:
                return json.loads(brace_match.group(0))
        except:
            pass
        
        # Method 4: Manual extraction for common patterns
        return self._extract_manual_data(cleaned_text)

    def _extract_manual_data(self, text: str) -> Dict[str, Any]:
        """Manual data extraction as last resort"""
        data = {}
        ...
        for pattern in fund_patterns:
            ...
            if matches:
                data["fund_name"] = matches[0].strip()
                break
        
        # Look for currency
        currency_matches = re.findall(r'USD|EUR|GBP|JPY|\$|€|£|¥', text)
        if currency_matches:
            data["fund_currency"] = currency_matches[0].replace('$', 'USD').replace('€', 'EUR').replace('£', 'GBP').replace('¥', 'JPY')
        
        # Look for numbers that could be fund size
        number_matches = re.findall(r'\$?(\d+(?:\.\d+)?)\s*(?:million|M|billion|B|trillion|T)', text, re.IGNORECASE)
        if number_matches:
            try:
                data["fund_size"] = float(number_matches[0])
            except:
                pass
        
        return data if data else {}

    def _extract_minimal_data(self, prompt: str, model_name: str) -> Dict[str, Any]:
        """Extract minimal data with even simpler prompt"""
        try:
            simple_prompt = """
            Extract JUST the fund name and currency from the text. 
            Return as JSON: {"fund_name": "name or null", "fund_currency": "currency or null"}
            """
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": simple_prompt + "\n\n" + prompt.split("DOCUMENT TEXT:")[1] if "DOCUMENT TEXT:" in prompt else prompt
                    }
                ],
                temperature=0.1,
                max_tokens=500,
            ...
            return {"fund_name": None, "fund_currency": None}

    def _is_valid_structure(self, data: Dict[str, Any]) -> bool:
        """Check if data has basic valid structure"""
        if not isinstance(data, dict):
            return False
        
        # Check if it has at least one meaningful field
        meaningful_fields = [k for k, v in data.items() if v not in [None, "", []] and k != "_metadata"]
        return len(meaningful_fields) > 0

    def _get_fallback_structure(self, template_id: int, error_msg: str) -> Dict[str, Any]:
        """Return fallback structure when extraction fails"""
        if template_id == 1:
            return {
                "fund_information": {
                    "fund_name": None,
